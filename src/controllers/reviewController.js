...
const createReview = async function (req, res) {
  try {
    ...
    const reviewinc = await bookModel.findByIdAndUpdate(bookId, {
      $inc: { reviews: 1 },
    });
    const alert = await bookModel
      .findById(bookId)
      .select({ createdAt: 0, updatedAt: 0, deletedAt: 0 });
    ...
    if (!rating)
      return res
        .status(400)
        .send({ status: false, messege: "rating is required" });
    ...
    if (!review)
      return res
        .status(400)
        .send({ status: false, messege: "review is required" });
...
const updatereview = async function (req, res) {
  try {
    ...
    if (!isValid1)
      return res
        .status(400)
        .send({ status: false, msg: "enter valid rejectID" });

    if (!reviewId)
      return res
        .status(400)
        .send({ status: false, messege: " reviewId is required" });
    ...
    if (alert2.isDeleted)
      return res
        .status(409)
        .send({ status: false, messege: "this review is already deleted" });

    if (alert2.bookId != reviewId)
      return res
        .status(400)
        .send({ status: false, messege: "book and review doesn't match " });
...
const deletereview = async (req, res) => {
  try {
    ...
    if (!isValid1)
      return res
        .status(400)
        .send({ status: false, msg: "enter valid reviewID" });
    if (!reviewId)
      return res
        .status(400)
        .send({ status: false, messege: " reviewId is required" });
