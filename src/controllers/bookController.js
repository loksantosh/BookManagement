...
const createBooks = async function (req, res) {
    try {
        ...
        if (typeof title !== "string"|| title.trim().length == 0) return res.status(400).send({ status: false, messege: " Please enter  title as a String" });
        ...
        if (typeof excerpt !== "string" || excerpt.trim().length == 0) return res.status(400).send({ status: false, messege: " Please enter  excerpt as a String" });
    ...
    try {
        ...
        if (bookId) {
            if (!mongoose.isValidObjectId(bookId)) {
                return res.status(400).send({ status: false, messege: "Please enter bookID as a valid ObjectId" })
            }
        }
...
const updateBook = async function (req, res) {
    ...
    try {
        ...
        if (!mongoose.isValidObjectId(bookId)) return res.status(400).send({ status: false, messege: "enter valid objectID" })
