...
const createUser = async function (req, res) {
  try {
    ...
    if (!(title == "Mrs" || title == "Mr" || title == "Miss")) {
      return res
        .status(400)
        .send({ error: "title has to be Mr or Mrs or Miss " });
    }
    ...
    if (typeof name != "string" || name.trim().length == 0)
      return res
        .status(400)
        .send({ status: false, msg: " Please enter  name as a String" });
    ...
    if (
      ...
      return res
        ...
        .send({
          status: false,
          msg: "password must be 8-15",
        });

    //address

    if (!/^\w[a-zA-Z.,\s]*$/.test(address.city))
      return res
        .status(400)
    ...
    return res
      ...
      .send({
        status: true,
        msg: "Your user has been created successfully",
        data: saveData,
      });
  } catch (err) {
...
const loginUser = async function (req, res) {
  try {
    ...
    if (Object.keys(req.body).length == 0) {
      return res
        .status(400)
        .send({ status: false, msg: "please enter data in request body" });
    }
    ...
    return res
      .status(200)
      .send({ status: true, msg: "user are login successfully", data: token });
  } catch (err) {
    return res.status(500).send({ status: false, msg: err.message });
  }
};
